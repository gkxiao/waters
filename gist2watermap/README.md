...
```bash
python csv2watermap_mae.py -o gist_fake_wm.mae --title 4zlz_apo-GIST apo_hs.csv
```

### Arguments

- `-o gist_fake_wm.mae` – output `.mae` file name.
...
After execution, a `gist_fake_wm.mae` file will be generated in the current directory. This file is Watermap-style Maestro structure output, with Watermap-compatible properties such as `r_watermap_deltaG` and `r_watermap_occupancy` written into it.
...
```pymol
load gist_fake_wm.mae
```

By default, the object name will be `gist_fake_wm` (if you use a different filename, the object name will change accordingly; use `get_names()` to check).

### 2. Display hydration sites as spheres

```pymol
cmd.show("spheres", "gist_fake_wm")
```

### 3. Color by ΔG value

```pymol
cmd.spectrum("properties['r_watermap_deltaG']", "green white red", "gist_fake_wm", minimum=-5, maximum=5)
```
...
```pymol
cmd.spectrum("properties['r_watermap_occupancy']", "blue white orange", "gist_fake_wm", minimum=0, maximum=1)
```

Display as dots:

```pymol
cmd.show("dots", "gist_fake_wm")
```

## Usage in Maestro / Ligand Designer

1. Open Schrödinger Maestro.
2. Import `gist_fake_wm.mae` via `File → Import`.
...
- If the object name in PyMOL is not `gist_fake_wm`, replace it with the actual object name in the commands above.
